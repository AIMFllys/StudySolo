...
| Agent（Phase 5） | `/api/agents/*` | `api/agents.py` | Agent Gateway 路由（待实现） |
...
├── _template/                  # Agent 模板 (复制创建新 Agent)
│   ├── main.py                 # Agent 入口模板
│   ├── config.yaml             # 配置模板
...
- **端口范围**: 8001-8099
- **协议**: HTTP REST + SSE
- **认证**: 共享 Supabase JWT 或独立 API Key
- **与主后端通信**: 通过 `backend/app/services/` 服务层调用
