...
const features = [
    {
        title: '明暗线双轨架构',
        subtitle: 'Explicit & Implicit Architecture',
        desc: '彻底解决传统线性工作流中 AI "精神分裂"的问题。暗线（总编）作为全局上下文 JSON 状态机，驻留在整个工作流生命周期中；明线（记者团队）是真正干活的单一节点。系统自动将暗线数据注入每一个明线节点的 System Prompt——不管拆分成多少细碎任务，输出风格始终一致、逻辑连贯。',
        details: ['暗线 = 全局主题 + 语言风格 + 核心大纲 + 受众定位', '明线 = 具体节点（写前言/正文/结论等）', '暗线可动态更新·明线零感知切换'],
    ...
    },
    {
        ...
        desc: 'AI 只是苦力，你才是真正的驾驶员。三步人机协同：暂停(Pause) → 人工覆写(Override) → 满血复活(Resume)。点击停止即秒变 paused 状态，直接修改 AI 输出文本，引擎带着你的正确修改继续执行下游节点。',
        details: ['异步任务队列 + 数据库实时状态字段', '随时审查输出·实时纠偏', '坚决不用"死等几分钟看盲盒结果"'],
    ...
    {
        title: '拆解聚合与格式化输出',
        subtitle: 'Map-Reduce & Export',
        desc: '打破大模型的单次 Token 输出限制。Map 阶段根据大纲自动生成多个独立子任务（可并行执行，极大缩短等待时间）；Reduce 阶段用专门的润色合并节点梳理上下文、消除重复话术、统一风格；Export 一键将 Markdown 转为排版精美的 PDF/DOCX。',
        details: ['Map 分发 → 并行 AI 生成', 'Reduce 聚合 → 润色消冗余', 'Export 实体化 → PDF / DOCX 下载'],
    ...
    },
    {
        ...
        desc: '告别全能黑盒。建立用户信任，区分"黑盒工具"与"专业系统"。WebSocket 长连接推送每一步进度，节点亮呼吸灯 → 完成打绿色对勾；原始 JSON 自动转译为友好标签（🔍 正在搜索：生物医药）；右侧边栏打字机模式实时渲染 Markdown 产出。',
        details: ['呼吸灯 UI × 实时状态推送', '数据脱水与标签化友好展示', 'Markdown 实时预览 × 打字机效果'],
